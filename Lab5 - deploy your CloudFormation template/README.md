...
## Scenario
In this lab, we'll deploy a ClooudFormation template at first. This template include a VPC with a single public subnet. Then, we'll deploy the next template to create an EC2 instance and security group. After that, we'll try to upgrade the template and see what it has changed. Last, we'll clean up the template we've created in this lab and watch the deletion of the template.
...
> If you are curious about the template file, prepare a text editor that can read **YAML** or **YML** like **Visual Studio Code**.
...
5. Find **CloudFormation-Network.yml**, which downloaded in prerequisites part and upload it.
...
9. Select **Create stack** and back to your stack list.
...
> The reources inside the template will be created in a few minutes.
...
Now we have our VPC environment setup, lets deploy an EC2 instance in this VPC.
...
4. Find **CloudFormation-App.yml** and upload it.
...
9. Wait until the template been created, Select **Outputs** tab.

10. Click the **URL value**, you'll see a webpage hosted in the EC2. It shows the details of your EC2.
...
Since we already have a VPC and EC2 instance, now we're going to make a little update. In this part, we'll update the template in previous step. Add a rule to security group and associate an elastic IP to EC2.
...
4. Find **CloudFormation-App-v2.yml** and upload it.
...
7. Select **Next** until the Review page.

8. Scroll down to the end of review page.

<p align="center">
    <img src="images/012-update.jpg" width="70%" height="70%">
</p>

9. Select **Update stack**.

10. Select **Events** tab and you can see the status of the template.
...
CloudFormation is easy to deploy and easy to clean up, just select your stack and click **Delete** button on the top. Please delete **Lab-App-Stack** first then delete **Lab-VPC-Stack**. While deleting, you can check the **events** tab and watch the deleting schedule.
